...
(define-ruleset radistributivity (arithmetic simplify)
  ...
  [rrename3- (r* (-/- u a b) (-/- v a b)) (-/- (r* u v) a b)]
  [rrename4a (-/- (agg a u) a b) (agg b (-/- u a b))]
  [rrename4a- (agg b (-/- u a b)) (-/- (agg a u) a b)]
  [rrename4b (-/- (agg (hasnt c a) u) a (hasnt b c)) (agg c (-/- u a b))]
  [rrename5 (-/- (-/- x a b) b c) (-/- x a c)]
  [rrename6 (-/- (-/- x a b) a c) (-/- x a b)]
  [rrename7 (-/- (-/- x (hasnt a c) (hasnt b c)) c (hasnt d a)) (-/- (-/- x c d) a b)]

  [rrename8 (-/- (b+ x ds) a b) (b+ x (-/- ds a b))]
  [rrename9 (-/- (: c d) a b) (: (-/- c a b) (-/- d a b))]
  [rrename10 (-/- a a b) b]
  [rrename10 (-/- (hasnt c a) a b) c]
  [swapattr (: i j) (: j i)]
