...
(define (type? x) (or (equal? x 'real) (equal? x 'bool) (equal? x 'complex)(equal? x 'rplan)(equal? x 'ds)(equal? x 'attr)(equal? x 'matrix)))
...
(define-operator (: attr attr) ds
  [fl +] [bf exact+] [cost 1]
  [->c/double (curry format "~a + ~a")]
  [->c/mpfr (curry format "mpfr_add(~a, ~a, ~a, MPFR_RNDN)")]
  [->tex (curry format "~a + ~a")][nonffi +])

(define-operator (b+ matrix ds) rplan
  [fl +] [bf exact+] [cost 1]
  [->c/double (curry format "~a + ~a")]
  [->c/mpfr (curry format "mpfr_add(~a, ~a, ~a, MPFR_RNDN)")]
  [->tex (curry format "~a + ~a")][nonffi +])

(define-operator (agg attr rplan) rplan
  [args '(2)] [type (hash 2 '(((attr rplan ) rplan )))]
  [fl +] [bf exact+] [cost 1]
  [->c/double (curry format "~a + ~a")]
  [->c/mpfr (curry format "mpfr_add(~a, ~a, ~a, MPFR_RNDN)")]
  [->tex (curry format "~a + ~a")][nonffi +])
...
(define-operator (r* rplan rplan) rplan
  [args '(2)] [type (hash 2 '(((rplan rplan ) rplan )))]
  [fl +] [bf exact+] [cost 1]
  [->c/double (curry format "~a + ~a")]
  [->c/mpfr (curry format "mpfr_add(~a, ~a, ~a, MPFR_RNDN)")]
  [->tex (curry format "~a + ~a")][nonffi +])
